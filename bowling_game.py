"""
Bowling Game Implementation (Fixed and Documented Version)
A module for calculating bowling game scores according to standard ten-pin bowling rules.

This implementation fixes several bugs found in the original code:
- Added proper 10th frame handling 
- Fixed incomplete _spare_bonus method
- Added input validation
- Corrected open frame scoring logic
...
class BowlingGame:
    """
    A ten-pin bowling game score calculator.
    
    This class handles the scoring logic for a complete bowling game,
    including strikes, spares, and the special tenth frame rules.
    
    Attributes:
        rolls (list): List of all rolls in the game
        current_roll (int): Index of the current roll
    ...
    def roll(self, pins):
        ...
        """
        # Input validation - fixed bug from original
    ...
    def score(self):
        ...
        # Process first 9 frames - fixed bug (was only processing 9 frames total)
        for frame in range(9):
            if self._is_strike(frame_index):
                ...
                # Open frame: sum of 2 rolls - fixed bug (was only adding first roll)
                score += self.rolls[frame_index] + self.rolls[frame_index + 1]
                frame_index += 2

        # Process 10th frame - added missing functionality
    ...
    def _score_tenth_frame(self, frame_index):
        ...
        """
        if self._is_strike(frame_index):
            # Strike in 10th: count all 3 rolls
            ...
            # Spare in 10th: count all 3 rolls
            if frame_index + 2 < len(self.rolls):
                return 10 + self.rolls[frame_index + 2]
            else:
                return 10
        else:
            # Open frame in 10th: count 2 rolls
    ...
    def _spare_bonus(self, frame_index):
        ...
        """
        # Fixed bug: original code was missing return statement
        if frame_index + 2 < len(self.rolls):
    ...
    def is_game_complete(self):
        ...
        if frame_index < len(self.rolls):
            if self.rolls[frame_index] == 10:  # Strike in 10th
                rolls_needed += 2
            elif (frame_index + 1 < len(self.rolls) and 
                  self.rolls[frame_index] + self.rolls[frame_index + 1] == 10):  # Spare in 10th
                rolls_needed += 1
